...
public class Shader implements IShader {
    ...
    public Shader(String vertexShader, String fragmentShader) {
        ...
        if (vertexShader != null) {
            GL20.glAttachShader(programID, createShader(vertexShader + FILE_EXT_VERTEX, GL20.GL_VERTEX_SHADER));
        }
        if (fragmentShader != null) {
            GL20.glAttachShader(programID, createShader(fragmentShader + FILE_EXT_FRAGMENT, GL20.GL_FRAGMENT_SHADER));
        }
    ...
    private int createShader(String string, int glVertexShader) {
        ...
        if (shaderStatus == GL11.GL_FALSE) {
            throw new RuntimeException("Compilation error for shader '" + string + "'. Reason: " + GL20.glGetShaderInfoLog(shaderID, GL20.glGetShaderi(shaderID, GL20.GL_INFO_LOG_LENGTH)));
        }
        return shaderID;
    }
